...
class PostQuerysetTemplateResponseMixin(TemplateResponseMixin):
    """
    Return a custom template name for views returning
    a queryset of Post filtered by another model.
    """
    model_type = None
    model_name = None
    ...
    def get_template_names(self):
        ...
        templates = [
            'blog/%s/%s/post_list.html' % (model_type, model_name),
            'blog/%s/%s_post_list.html' % (model_type, model_name),]
...
class PostQuerysetArchiveTemplateResponseMixin(TemplateResponseMixin):
    """
    ...
    def get_default_base_template_names(self):
        ...
        """
        return ['post%s.html' % self.template_name_suffix]
...
class PostQuerysetArchiveTodayTemplateResponseMixin(
        PostQuerysetArchiveTemplateResponseMixin):
    """
    Same as PostQuerysetArchiveTemplateResponseMixin
...
class PostArchiveTemplateResponseMixin(
        PostQuerysetArchiveTemplateResponseMixin):
    """
    Same as PostQuerysetArchiveTemplateResponseMixin
    but use the template defined in the Post instance
    as the base template name.
    """

    def get_default_base_template_names(self):
        """
        Return the Post.template value.
        """
