...
class TagPostListView(BasePostListView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.tag = get_object_or_404(Tag, name=self.kwargs.get('name'))
    ...
    def get_queryset(self):
        post_list = self.tag.post_set.filter(status=1)
...
class PostDetailView(SetHeadlineMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            previous_post = Post.objects.filter(status=1,created_time__gt=post.created_time).order_by("created_time").first()
        except Post.DoesNotExist:
            previous_post = None

        try:
            next_post = Post.objects.filter(status=1,created_time__lt=post.created_time).order_by("created_time").first()
        except Post.DoesNotExist:
            next_post = None
